...
if ($_GET['rd-clr']) {
	setcookie('rd-clr', $_GET['rd-clr'], time()+60*60*24*30, '/');
...
// ENQUEUE SCRIPTS
	function loadScripts(){
		...
		wp_enqueue_script('common-scripts', get_bloginfo('template_directory') . '/scripts/common.js', array('jquery','jquery-ui'));
	}
	
	// ENQUEUE STYLES
	function loadStyles(){
		wp_enqueue_style('common-styles', get_bloginfo('stylesheet_url'), array(), false, 'all');
	}
/**
...
function toolbox_setup() {
	 ...
	 * to change 'wp_responds' to the name of your theme in all the template files
	 */
	...
	register_nav_menus( array(
		'banner' => __( 'Banner Menu', 'wp_responds' ),
		'column' => __( 'Column Menu','wp_responds' ),		
	) );
...
function toolbox_widgets_init() {
	...
	register_sidebar( array(
		'name' => __( 'Sidebar 2', 'wp_responds' ),
		'id' => 'sidebar-2',
		'description' => __( 'An optional second sidebar area', 'wp_responds' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => "</aside>",
		'before_title' => '<h1 class="widget-title">',
		'after_title' => '</h1>',
...
function toolbox_content_nav( $nav_id ) {
	...
	<nav id="<?php echo $nav_id; ?>">
		<h1 class="assistive-text"><?php _e( 'Post navigation', 'wp_responds' ); ?></h1>

	<?php if ( is_single() ) : // navigation links for single posts ?>

		<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '&larr;', 'Previous post link', 'wp_responds' ) . '</span> %title' ); ?>
		<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next post link', 'wp_responds' ) . '</span>' ); ?>

	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>

		<?php if ( get_next_posts_link() ) : ?>
		<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'wp_responds' ) ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'wp_responds' ) ); ?></div>
		<?php endif; ?>
...
function toolbox_comment( $comment, $args, $depth ) {
		...
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			<footer>
				<div class="comment-author vcard">
					...
					<?php printf( __( '%s <span class="says">says:</span>', 'wp_responds' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
				</div><!-- .comment-author .vcard -->
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<em><?php _e( 'Your comment is awaiting moderation.', 'wp_responds' ); ?></em>
					<br />
				<?php endif; ?>
				...
				<div class="comment-meta commentmetadata">
					...
					<?php
						...
						printf( __( '%1$s at %2$s', 'wp_responds' ), get_comment_date(), get_comment_time() ); ?>
					</time></a>
					<?php edit_comment_link( __( '(Edit)', 'wp_responds' ), ' ' );
					?>
	...
	<li class="post pingback">
		<p><?php _e( 'Pingback:', 'wp_responds' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( '(Edit)', 'wp_responds' ), ' ' ); ?></p>
	<?php
			break;
	endswitch;
}
 ...
 * @since WP RESPONDS 0.1
 */
function rd_body_classes( $classes ) {
	...
	if ($_GET['rd-clr']) {
		$classes[] = $_GET['rd-clr'];
	} elseif ($_COOKIE['rd-clr']) {
		$classes[] = $_COOKIE['rd-clr'];
	}

	return $classes;
}
add_filter( 'body_class', 'rd_body_classes' );
 ...
 */
	...
	function my_search_form( $form ) {
	    ...
	    <fieldset><label class="screen-reader-text" for="s">' . __('Search for:') . '</label>
 ...
 */
	...
	function is_mobile() {
		...
		$svg_mobile_devices = array('Android', 'iPod', 'iPhone', 'iPad', 'webOS'/*, 'Gecko'*/);
		
		foreach ($svg_mobile_devices as $device) {
...
/**
 *  Button Up of Button Down
 */
 
	function button_up_or_down($button) {
		if ($_GET['rd-clr']) {
			$down = $_GET['rd-clr'];
		} elseif ($_COOKIE['rd-clr']) {
			$down = $_COOKIE['rd-clr'];
...
/**
 *  Add class to images with links
 */	
 	function my_image_send_to_editor($html) {
 		$html = substr($html, 0, 2) . ' class="image-link"' . substr($html, 2);
 		return $html;
 	}
 
	add_filter('image_send_to_editor', 'my_image_send_to_editor');